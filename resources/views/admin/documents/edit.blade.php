                ...
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.documents.update", [$document->id]) }}" enctype="multipart/form-data">
                        ...
                        <div class="form-group {{ $errors->has('reads') ? 'has-error' : '' }}">
                            <label for="reads">{{ trans('cruds.document.fields.read') }}</label>
                            ...
                            <select class="form-control select2" name="reads[]" id="reads" multiple>
                                @foreach($reads as $id => $read)
                                    <option value="{{ $id }}" {{ (in_array($id, old('reads', [])) || $document->reads->contains($id)) ? 'selected' : '' }}>{{ $read }}</option>
                                @endforeach
                            </select>
                            @if($errors->has('reads'))
                                <span class="help-block" role="alert">{{ $errors->first('reads') }}</span>
                            @endif
                            <span class="help-block">{{ trans('cruds.document.fields.read_helper') }}</span>
                        </div>
