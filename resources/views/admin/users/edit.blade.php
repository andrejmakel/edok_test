            ...
            <div class="panel panel-default">
                ...
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.users.update", [$user->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group {{ $errors->has('archiv') ? 'has-error' : '' }}">
                            <div>
                                ...
                                <label for="archiv" style="font-weight: 400">{{ trans('cruds.user.fields.archiv') }}</label>
                            </div>
                            @if($errors->has('archiv'))
                                <span class="help-block" role="alert">{{ $errors->first('archiv') }}</span>
                            @endif
                        ...
                        <div class="form-group {{ $errors->has('greeting') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="greeting" id="greeting" value="{{ old('greeting', $user->greeting) }}">
                        ...
                        <div class="form-group {{ $errors->has('notifications') ? 'has-error' : '' }}">
                            <div>
                                ...
                                <input type="checkbox" name="notifications" id="notifications" value="1" {{ $user->notifications || old('notifications', 0) === 1 ? 'checked' : '' }}>
                        ...
                        <div class="form-group {{ $errors->has('teams') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="teams[]" id="teams" multiple>
                                @foreach($teams as $id => $team)
                                    <option value="{{ $id }}" {{ (in_array($id, old('teams', [])) || $user->teams->contains($id)) ? 'selected' : '' }}>{{ $team }}</option>
                                @endforeach
                            </select>
                            @if($errors->has('teams'))
                        ...
                        <div class="form-group {{ $errors->has('lang') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="lang_id" id="lang_id">
                                @foreach($langs as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('lang_id') ? old('lang_id') : $user->lang->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                            @if($errors->has('lang'))
                        ...
                        <div class="form-group {{ $errors->has('whats_app') ? 'has-error' : '' }}">
                            <div>
                                ...
                                <label for="whats_app" style="font-weight: 400">{{ trans('cruds.user.fields.whats_app') }}</label>
                            </div>
                        ...
                        <div class="form-group {{ $errors->has('notice') ? 'has-error' : '' }}">
                            ...
                            <textarea class="form-control ckeditor" name="notice" id="notice">{!! old('notice', $user->notice) !!}</textarea>
