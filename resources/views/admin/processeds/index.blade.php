...
<div class="content">
    @can('processed_create')
        <div style="margin-bottom: 10px;" class="row">
            <div class="col-lg-12">
                <a class="btn btn-success" href="{{ route('admin.processeds.create') }}">
                            ...
                            <tbody>
                                @foreach($processeds as $key => $processed)
                                    <tr data-entry-id="{{ $processed->id }}">
                                        ...
                                        <td>
                                            @can('processed_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('admin.processeds.show', $processed->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('processed_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('admin.processeds.edit', $processed->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('processed_delete')
                                                <form action="{{ route('admin.processeds.destroy', $processed->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('processed_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.processeds.massDestroy') }}",
