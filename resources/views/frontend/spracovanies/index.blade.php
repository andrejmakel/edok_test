        ...
        <div class="col-md-12">
            @can('spracovany_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.spracovanies.create') }}">
                            ...
                            <tbody>
                                @foreach($spracovanies as $key => $spracovany)
                                    <tr data-entry-id="{{ $spracovany->id }}">
                                        ...
                                        <td>
                                            @can('spracovany_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.spracovanies.show', $spracovany->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('spracovany_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.spracovanies.edit', $spracovany->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('spracovany_delete')
                                                <form action="{{ route('frontend.spracovanies.destroy', $spracovany->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('spracovany_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.spracovanies.massDestroy') }}",
