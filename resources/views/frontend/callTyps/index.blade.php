        ...
        <div class="col-md-12">
            @can('call_typ_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.call-typs.create') }}">
                            ...
                            <tbody>
                                @foreach($callTyps as $key => $callTyp)
                                    <tr data-entry-id="{{ $callTyp->id }}">
                                        ...
                                        <td>
                                            @can('call_typ_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.call-typs.show', $callTyp->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('call_typ_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.call-typs.edit', $callTyp->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('call_typ_delete')
                                                <form action="{{ route('frontend.call-typs.destroy', $callTyp->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('call_typ_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.call-typs.massDestroy') }}",
