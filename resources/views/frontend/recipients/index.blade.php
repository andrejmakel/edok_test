        ...
        <div class="col-md-12">
            @can('recipient_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.recipients.create') }}">
                            ...
                            <tbody>
                                @foreach($recipients as $key => $recipient)
                                    <tr data-entry-id="{{ $recipient->id }}">
                                        ...
                                        <td>
                                            @can('recipient_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.recipients.show', $recipient->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('recipient_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.recipients.edit', $recipient->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('recipient_delete')
                                                <form action="{{ route('frontend.recipients.destroy', $recipient->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('recipient_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.recipients.massDestroy') }}",
