        ...
        <div class="col-md-12">
            @can('outgoing_post_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.outgoing-posts.create') }}">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <div class="table-responsive">
                        <table class=" table table-bordered table-striped table-hover datatable datatable-OutgoingPost">
                            <thead>
                                <tr>
                                    <th>
                                    ...
                                    <tr data-entry-id="{{ $outgoingPost->id }}">
                                        ...
                                        <td>
                                            @can('outgoing_post_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.outgoing-posts.show', $outgoingPost->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('outgoing_post_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.outgoing-posts.edit', $outgoingPost->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('outgoing_post_delete')
                                                <form action="{{ route('frontend.outgoing-posts.destroy', $outgoingPost->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('outgoing_post_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.outgoing-posts.massDestroy') }}",
...
@endcan
  ...
  let table = $('.datatable-OutgoingPost:not(.ajaxTable)').DataTable({ buttons: dtButtons })
